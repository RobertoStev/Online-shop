...
class OnlineShop {
    ...
    OnlineShop() {
        productsById = new HashMap<>(); //O(1) kompleksnost
    ...
    void addProduct(String category, String id, String name, LocalDateTime createdAt, double price) {
        ...
        productsByCategory.putIfAbsent(category, new ArrayList<>()); //ako ja nema kategorijata stavi prazna lista na toa mesto
        //                  O(1)
        productsByCategory.get(category).add(p); //ke go dodade p vo soodvetnata lista
        //                  O(1)          O(1)
    }
    ...
    double buyProduct(String id, int quantity) throws ProductNotFoundException {
        ...
        return product.buy(quantity); //vrakja suma sto treba da se plati
        //return 0.0;
    }
    ...
    List<List<Product>> listProducts(String category, COMPARATOR_TYPE comparatorType, int pageSize) {
        ...
        if (category == null) {
            allProducts.addAll(productsById.values()); //gi dodava site vrednosti (produkti) od productsById
        } else {
            allProducts = productsByCategory.get(category); //ke gi vrati site proizvodi od taa kategorija
        }

        Comparator<Product> comparator = createComparator(comparatorType);

        Collections.sort(allProducts, comparator); //ke ja sortira listata spored comparatorot
        //allProducts.sort(comparator);

        for (int start = 0; start < allProducts.size(); start += pageSize) {
            int end = start + pageSize;
            if (end > allProducts.size()) {
                end = allProducts.size();
            }
            result.add(allProducts.subList(start, end));
        }
        return result;
    }
